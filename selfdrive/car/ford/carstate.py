...
class CarState(CarStateBase):
  def update(self, cp):
    ...
    ret.cruiseState.speed = cp.vl["Cruise_Status"]['Set_Speed'] * CV.KPH_TO_MS
    ...
    ret.genericToggle = bool(cp.vl["Steering_Buttons"]["Lane_Keep_Toggle"])
    ret.doorOpen = any([cp.vl["Doors"]["Door_FL_Open"], cp.vl["Doors"]["Door_FR_Open"], 
                        cp.vl["Doors"]["Door_RL_Open"], cp.vl["Doors"]["Door_RR_Open"]])
    ret.leftBlinker = bool(cp.vl["Steering_Buttons"]["Left_Turn_Light"])
